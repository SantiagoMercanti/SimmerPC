...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: 'Error al obtener sensores', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Error al crear sensor', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
